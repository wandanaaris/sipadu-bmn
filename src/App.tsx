...
import { archiveSubmissionToDrive, createOpenSubmission, createTask, documentPreviewUrl, loadSubmissions, loadTasks, persistAssignmentStatus, persistTaskActive, reviewSubmission, submitLinkSubmission, transferSubmission, type SubmissionRecord } from './lib/repository'
  ...
  return <div className="admin-layout">
    ...
    <main className="main">
      ...
      <section className="panel monitoring">
        ...
        <div className="table-wrap"><table><thead><tr><th>Satker</th><th>Pekerjaan</th><th>Progress</th><th>Status</th><th>Pembaruan</th><th></th></tr></thead><tbody>{filtered.slice(0,12).map(x=><tr key={`${x.task.id}-${x.satker}`}><td><strong>{satkers.find(s=>s.code===x.satker)?.name}</strong><span>{x.satker}</span></td><td>{x.task.title}<small>{methodMeta[x.task.method].label}</small></td><td><Progress value={x.progress}/></td><td><StatusPill status={x.status}/></td><td>{x.updated}</td><td><button className="link-button" onClick={()=>setDetail(x.task.id)}>Periksa</button></td></tr>)}</tbody></table></div>
      </section>
...
function AdminSection({page,tasks,onRefresh,setDetail,toggleTask}:{page:Exclude<AdminPage,'summary'>;tasks:Task[];onRefresh:()=>Promise<void>;setDetail:(v:string|null)=>void;toggleTask:(id:string)=>void}){
 ...
 if(page==='monitoring') return <section className="panel admin-page"><div className="panel-head"><div><h2>Status seluruh satker</h2><p>{assignments.length} penugasan aktif dari {active.length} pekerjaan.</p></div></div><div className="table-wrap"><table><thead><tr><th>Satker</th><th>Pekerjaan</th><th>Progress</th><th>Status</th><th>Pembaruan</th><th></th></tr></thead><tbody>{assignments.slice(0,40).map(a=><tr key={`${a.task.id}-${a.satker}`}><td><strong>{satkers.find(s=>s.code===a.satker)?.name}</strong><span>{a.satker}</span></td><td>{a.task.title}<small>{methodMeta[a.task.method].label}</small></td><td><Progress value={a.progress}/></td><td><StatusPill status={a.status}/></td><td>{a.updated}</td><td><button className="link-button" onClick={()=>setDetail(a.task.id)}>Periksa</button></td></tr>)}</tbody></table></div></section>
 if(page==='verification') return <SubmissionInbox onTasksChanged={onRefresh}/>
...
function SubmissionInbox({onTasksChanged}:{onTasksChanged:()=>Promise<void>}){
...
function SatkerTaskDetail({item,onBack,flash}:{item:{task:Task;assignment:Task['assignments'][number]};onBack:()=>void;flash:(s:string)=>void}){
 ...
 return <div className="task-detail-page"><button className="back" onClick={onBack}><ArrowLeft/>Kembali ke daftar pekerjaan</button><div className="detail-heading"><div className={`method-icon method-${task.method}`}><meta.Icon/></div><div><span>{meta.label}</span><h1>{task.title}</h1><p>{task.description}</p></div><StatusPill status={assignment.status}/></div><div className="detail-layout"><section className="panel detail-content"><div className="detail-progress"><div><span>Progress pengisian</span><strong>{assignment.progress}%</strong></div><div className="progress-track"><i style={{width:`${assignment.progress}%`}}/></div></div>{assignment.missing.length>0&&<div className="missing-box"><CircleAlert/><div><strong>Data yang masih perlu dilengkapi</strong><ul>{assignment.missing.map(m=><li key={m}>{m}</li>)}</ul></div></div>}{task.method==='spreadsheet'?<div className="external-work"><FileSpreadsheet/><div><h3>Pengisian dilakukan pada spreadsheet sumber</h3><p>Gunakan tombol berikut agar selalu membuka tautan pekerjaan yang benar. Setelah selesai, kembali ke portal dan ajukan verifikasi.</p><a href={task.link} target="_blank" className="primary">Buka spreadsheet <ExternalLink/></a></div></div>:task.method==='upload'?<OpenUploadForm task={task} satkerCode={assignment.satker}/>:<PortalForm method={task.method}/>}{task.method!=='upload'&&<div className="submit-row"><button className="primary" onClick={handleSubmitLink}>Ajukan untuk diverifikasi</button><span>Data tidak langsung dinyatakan selesai sebelum diperiksa Korwil.</span></div>}</section><aside className="panel detail-side"><h3>Informasi pekerjaan</h3><dl><div><dt>Dasar</dt><dd>{task.letter}</dd></div><div><dt>Batas waktu</dt><dd>{task.due}</dd></div><div><dt>Terakhir diperbarui</dt><dd>{assignment.updated}</dd></div><div><dt>Metode</dt><dd>{meta.label}</dd></div></dl><div className="privacy"><ShieldCheck/><p>Data hanya digunakan untuk monitoring pekerjaan BMN Kanwil Ditjenpas Riau.</p></div></aside></div></div>
}
...
export function OpenUploadForm({task,satkerCode}:{task:Task;satkerCode:string}){
 ...
 const [senderName,setSenderName]=useState(''),[senderNote,setSenderNote]=useState(''),[files,setFiles]=useState<Record<string,File>>({}),[loading,setLoading]=useState(false),[error,setError]=useState(''),[receipt,setReceipt]=useState<{submissionNumber:string;documentCount:number}|null>(null)
 const submit=async(e:FormEvent)=>{e.preventDefault();const limit=(r:typeof requirements[number])=>r.accept?.includes('spreadsheet')?35*1024*1024:4*1024*1024;const tooBig=requirements.filter(r=>files[r.key]&&files[r.key].size>limit(r));if(tooBig.length){setError(`Ukuran file melebihi batas (PDF maks 4 MB, XLSX maks 35 MB): ${tooBig.map(x=>x.label).join(', ')}`);return}const missing=requirements.filter(r=>r.required!==false&&!files[r.key]);if(missing.length){setError(`Pilih file: ${missing.map(x=>x.label).join(', ')}`);return}setLoading(true);setError('');try{const selected=requirements.filter(r=>files[r.key]).map(r=>({file:files[r.key],requirementKey:r.key,documentType:r.label.replace(/\s*\([^)]*\)\s*/g,' ').replace(/\s+/g,' ').trim()}));const result=await createOpenSubmission({task,satkerCode,senderName,senderPhone:'',senderNote,files:selected});setReceipt(result)}catch(err){setError(err instanceof Error?err.message:'Pengajuan belum dapat dikirim.')}finally{setLoading(false)}}
 ...
 return <form className="open-upload-form" onSubmit={submit}><div className="upload-heading"><Upload/><div><h3>Unggah Data Dukung</h3><p>File masuk ke inbox privat dan baru dinyatakan sah setelah diverifikasi Korwil.</p></div></div>{task.references&&task.references.length>0&&<div className="reference-links"><FileSpreadsheet/><div><strong>Peraturan &amp; Format Data Dukung</strong><span>Unduh berkas panduan sebelum mengunggah:</span><div className="reference-buttons">{task.references.map(r=><a key={r.url} href={r.url} target="_blank" rel="noopener noreferrer" className="ghost">{r.label}<ExternalLink/></a>)}</div></div></div>}<div className="sender-grid"><label>Nama pengirim<input required value={senderName} onChange={e=>setSenderName(e.target.value)} placeholder="Nama operator/pengirim"/></label><label className="full">Keterangan<textarea value={senderNote} onChange={e=>setSenderNote(e.target.value)} placeholder="Keterangan tambahan (opsional)"/></label></div><div className="requirement-files">{requirements.map(r=><label className={files[r.key]?'has-file':''} key={r.key}><div><strong>{r.label}</strong><span>{files[r.key]?`${files[r.key].name} · ${(files[r.key].size/1024/1024).toFixed(2)} MB`:(r.accept?.includes('pdf')?'PDF':r.accept?.includes('spreadsheet')?'XLSX':'PDF, DOCX, XLSX, JPG, PNG, WebP, atau ZIP')+` · maks. ${r.accept?.includes('spreadsheet')?'35':'4'} MB`}</span></div><input type="file" aria-required={r.required!==false} accept={r.accept??".pdf,.docx,.xlsx,.jpg,.jpeg,.png,.webp,.zip"} onChange={e=>{const file=e.target.files?.[0];if(file)setFiles(current=>({...current,[r.key]:file}))}}/><b>{files[r.key]?'Ganti file':'Pilih file'}</b></label>)}</div>{error&&<div className="auth-message error">{error}</div>}<button className="primary upload-submit" disabled={loading}>{loading?'Mengunggah dan memfinalisasi…':'Unggah dan Ajukan ke Korwil'}</button><small className="upload-disclaimer">Dengan mengirim, pengunggah menyatakan file telah dipilih untuk satker dan pekerjaan yang benar.</small></form>
}
