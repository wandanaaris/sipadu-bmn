...
type DbAssignment = {
  ...
  satkers: { code: string } | { code: string }[] | null
}
type DbRequirement = { requirement_key:string; label:string; track:string|null; is_required:boolean }
type DbTask = {
  task_key: string
  title: string
  ...
  task_requirements: DbRequirement[] | null
  task_assignments: DbAssignment[] | null
}
...
const finalById=new Map(finalTasks.map(t=>[t.id,{title:t.title,method:t.method,requirements:t.requirements,assignments:t.assignments,references:t.references}]))
function applyFinalOverrides(tasks:Task[]):Task[]{
  if(!import.meta.env.DEV)return tasks
  return tasks.map(t=>{
    if(t.id!=='rkbmn')return t
    const o=finalById.get('rkbmn')
    return {...t,title:o?.title??t.title,method:o?.method??t.method,requirements:o?.requirements??t.requirements,references:o?.references??t.references,link:o?.method==='upload'?undefined:t.link,
      assignments:t.assignments.map(a=>({satker:a.satker,progress:0,status:'belum' as const,missing:[],revisionCount:0,updated:'Belum diperbarui',submittedAt:undefined,completedAt:undefined}))}
  })
}
export async function loadTasks(): Promise<{ tasks: Task[]; source: 'supabase' | 'fallback' }> {
  ...
  if (!authData.session) {const { data, error } = await supabase.rpc('get_active_portal');if (!error && Array.isArray(data) && data.length) return { tasks: applyFinalOverrides(data as Task[]), source: 'supabase' };console.warn('RPC portal aktif belum dapat dibaca; memakai data cadangan.', error?.message);return { tasks: fallback(), source: 'fallback' }}
  ...
  return {tasks:applyFinalOverrides((data as unknown as DbTask[]).map(mapTask)),source:'supabase'}
}
...
export async function submitLinkSubmission(input:{task:Task;satkerCode:string;senderName:string;senderPhone:string;senderNote:string;sheetUrl:string}){
...
export async function reviewSubmission(id:string,status:'diterima'|'perlu_perbaikan'|'ditolak',note:string){if(!supabase)throw new Error('Database belum tersedia.');const{data:submission,error:readError}=await supabase.from('submissions').select('assignment_id').eq('id',id).single();if(readError)throw readError;const now=new Date().toISOString();const{error}=await supabase.from('submissions').update({status,review_note:note||null,reviewed_at:now}).eq('id',id);if(error)throw error;const docStatus=status==='diterima'?'diterima':status==='perlu_perbaikan'?'perlu_perbaikan':'ditolak';await supabase.from('supporting_documents').update({verification_status:docStatus,review_note:note||null,archive_status:status==='diterima'?'pending_drive':'inbox'}).eq('submission_id',id);const assignmentStatus:TaskStatus=status==='diterima'?'selesai':status==='perlu_perbaikan'?'perbaikan':'belum';await supabase.from('task_assignments').update({status:assignmentStatus,completed_at:status==='diterima'?now:null,updated_at:now}).eq('id',submission.assignment_id);if(status==='diterima'){try{await archiveSubmissionToDrive(id)}catch(err){console.warn('Arsip otomatis ke Drive belum berhasil:',err instanceof Error?err.message:err)}}}
export async function archiveSubmissionToDrive(id:string){if(!supabase)throw new Error('Database belum tersedia.');const{data,error}=await supabase.functions.invoke('archive-to-drive',{body:{submissionId:id}});if(error)throw new Error(error.message);if(data?.error)throw new Error(data.error);return data as {archivedCount:number}}

export async function transferSubmission(id:string,targetCode:string,note:string){if(!supabase)throw new Error('Database belum tersedia.');const[{data:submission},{data:satker}]=await Promise.all([supabase.from('submissions').select('task_id').eq('id',id).single(),supabase.from('satkers').select('id').eq('code',targetCode).single()]);if(!submission||!satker)throw new Error('Satker tujuan tidak ditemukan.');const{data:assignment}=await supabase.from('task_assignments').select('id').eq('task_id',submission.task_id).eq('satker_id',satker.id).maybeSingle();if(!assignment)throw new Error('Satker tujuan tidak ditugaskan pada pekerjaan ini.');await supabase.from('submissions').update({satker_id:satker.id,assignment_id:assignment.id,status:'dialihkan',review_note:note||'Dialihkan oleh Korwil',reviewed_at:new Date().toISOString()}).eq('id',id);await supabase.from('supporting_documents').update({assignment_id:assignment.id}).eq('submission_id',id)}
